	...
	jQuery.post(Think.U('Home/UserAddress/getUserAddress') ,{addressId:addressId},function(rsp) {
		...
		if(rs.status>0){
			var addressInfo = rs.address;
                        var info=rs.info;
                        //alert(info.areaName1+info.areaName2+info.areaName3+info.address);
                        //$("#consignee_add_cityName").val(info.areaName1+info.areaName2+info.areaName3+info.address);
			//$("#consignee_add_cityName").val(addressInfo.areaName);
                        $("#consignee_add_cityName").val(addressInfo.a+addressInfo.b+addressInfo.c);
			...
			//$("#consignee_add_countyId").val(addressInfo.areaId1);
                        $("#areaId1").val(addressInfo.areaId1);
			...
			for(var i=0;i<communitys.length;i++){
				...
				if(community.areaId == addressInfo.areaId2){
					areaList.push("<option value="+community.areaId+" selected='selected'>"+community.areaName+"</option>");
				}else{
					areaList.push("<option value="+community.areaId+" >"+community.areaName+"</option>");
				}
			}
			$("#areaId2").html(areaList.join(""));
