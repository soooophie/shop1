...
class UserAddressModel extends BaseModel {
	...
	public function getUserAddressInfo(){
		...
		/*$sql ="SELECT ad.*, a.areaName FROM __PREFIX__user_address ad
		      ...
		      WHERE ad.addressId=$addressId AND ad.addressFlag=1 and ad.userId=".(int)session('WST_USER.userId');*/
                $sql ="SELECT ad.*, a.areaName as a,b.areaName as b,c.areaName as c FROM __PREFIX__user_address ad
			  left join __PREFIX__areas a ON ad.areaId1 = a.areaId left join __PREFIX__areas b ON ad.areaId2 = b.areaId left join __PREFIX__areas c ON ad.areaId3 = c.areaId
		      WHERE ad.addressId=$addressId AND ad.addressFlag=1 and ad.userId=".(int)session('WST_USER.userId');
		...
		//$collegesList = self::getCommunitys($rs["areaId3"]);
                $collegesList = self::getCommunitys($rs["areaId1"]);
	...
	public function getCommunitys($districtId){	
		//$sql = "SELECT * FROM __PREFIX__communitys WHERE areaId3=$districtId  AND isShow =1 ORDER BY communitySort";	
                $sql = "SELECT * FROM __PREFIX__areas WHERE parentId=$districtId  AND areaFlag = 1 AND areaType = 1 AND isShow =1";		
		$rs = $this->query($sql);		
		return $rs;
	}	
	...
	public function getAddressDetails($addressId){
		...
		if(empty($address))return array();
                ...
                $areaId1 = $address["areaId1"];
		$areaId2 = $address["areaId2"];
		$areaId3 = $address["areaId3"];
		$communityId = $address["communityId"];
                
                $sql = "SELECT areaId ,areaName FROM __PREFIX__areas WHERE areaId=$areaId1  AND areaFlag = 1 AND isShow =1";		
		$rs = $this->queryRow($sql);
		$sheng = $rs["areaName"];//省
		...
		$address["paddress"] =$sheng ." ". $cityName ." ". $districtsName ." ". $communityName;
		return $address ;
	}
